...
const solved_ac_strategy_1 = require("./solved-ac.strategy");
describe('SolvedAcStrategy', () => {
    ...
    beforeEach(() => {
        strategy = new solved_ac_strategy_1.SolvedAcStrategy();
    });
    describe('metadata', () => {
    ...
    describe('isApplicable', () => {
        it('should return false when no solvedAcProfile', () => {
            expect(strategy.isApplicable(baseCandidate)).toBe(false);
        });
        it('should return true when solvedAcProfile exists', () => {
            ...
            expect(strategy.isApplicable(candidate)).toBe(true);
        });
    });
    describe('calculate', () => {
        it('should return 0 when no profile', async () => {
            const result = await strategy.calculate(baseCandidate);
        ...
        it('should reward advanced algorithm knowledge', async () => {
            ...
            const advancedResult = await strategy.calculate(candidateWithAdvanced);
            const basicResult = await strategy.calculate(candidateWithoutAdvanced);
            expect(advancedResult.breakdown?.algorithmDepth).toBeGreaterThan(basicResult.breakdown?.algorithmDepth || 0);
        });
        it('should reward consistency (max streak)', async () => {
            ...
            const result = await strategy.calculate(consistentCandidate);
...
//# sourceMappingURL=solved-ac.strategy.spec.js.map