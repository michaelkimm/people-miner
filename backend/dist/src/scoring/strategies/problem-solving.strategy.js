...
exports.ProblemSolvingStrategy = void 0;
const common_1 = require("@nestjs/common");
let ProblemSolvingStrategy = class ProblemSolvingStrategy {
    constructor() {
        this.name = 'problemSolving';
        this.description = 'Scores based on problem-solving indicators: algorithms, OSS contributions';
    ...
    async calculate(candidate) {
        ...
        const value = Math.round(algorithmScore * 0.30 +
            sourceScore * 0.30 +
    ...
    scoreAlgorithmPractice(repositories) {
        const algorithmRepos = repositories.filter(r => this.algorithmPatterns.some(pattern => pattern.test(r.name) || (r.description && pattern.test(r.description))));
        if (algorithmRepos.length === 0)
            return 40;
        let score = 50;
        ...
        if (hasStarredAlgo)
            score += 10;
        const wellMaintained = algorithmRepos.filter(r => r.description && r.description.length > 20);
        if (wellMaintained.length > 0)
            score += 10;
        return Math.min(score, 100);
    }
    scoreSourceQuality(sources) {
        if (sources.length === 0)
            return 40;
        let score = 50;
        const fromPrestigious = sources.filter(s => this.prestigiousOrgs.has(s.sourceName.toLowerCase()));
    ...
    scoreProjectComplexity(repositories) {
        ...
        const complexProjects = repositories.filter(r => complexPatterns.some(pattern => pattern.test(r.name) || (r.description && pattern.test(r.description))));
    ...
    scoreProblemDiversity(repositories) {
...
exports.ProblemSolvingStrategy = ProblemSolvingStrategy;
exports.ProblemSolvingStrategy = ProblemSolvingStrategy = __decorate([
    (0, common_1.Injectable)()
], ProblemSolvingStrategy);
//# sourceMappingURL=problem-solving.strategy.js.map