...
const activity_strategy_1 = require("./activity.strategy");
describe('ActivityStrategy', () => {
    ...
    beforeEach(() => {
        strategy = new activity_strategy_1.ActivityStrategy();
    });
    describe('metadata', () => {
    ...
    describe('calculate', () => {
        ...
        it('should score repository count correctly', async () => {
            expect((await strategy.calculate({ ...baseCandidate, publicRepos: 50 })).breakdown?.repositories).toBe(100);
            expect((await strategy.calculate({ ...baseCandidate, publicRepos: 30 })).breakdown?.repositories).toBe(85);
            expect((await strategy.calculate({ ...baseCandidate, publicRepos: 20 })).breakdown?.repositories).toBe(70);
            expect((await strategy.calculate({ ...baseCandidate, publicRepos: 10 })).breakdown?.repositories).toBe(55);
            expect((await strategy.calculate({ ...baseCandidate, publicRepos: 5 })).breakdown?.repositories).toBe(40);
            expect((await strategy.calculate({ ...baseCandidate, publicRepos: 2 })).breakdown?.repositories).toBe(16);
        });
        it('should score commit activity correctly', async () => {
            expect((await strategy.calculate({ ...baseCandidate, totalCommits: 1000 })).breakdown?.commits).toBe(100);
            expect((await strategy.calculate({ ...baseCandidate, totalCommits: 500 })).breakdown?.commits).toBe(85);
            expect((await strategy.calculate({ ...baseCandidate, totalCommits: 200 })).breakdown?.commits).toBe(70);
            expect((await strategy.calculate({ ...baseCandidate, totalCommits: 100 })).breakdown?.commits).toBe(55);
            expect((await strategy.calculate({ ...baseCandidate, totalCommits: 50 })).breakdown?.commits).toBe(40);
        });
        it('should score language diversity', async () => {
            ...
            expect((await strategy.calculate(fiveLanguages)).breakdown?.languageDiversity).toBe(100);
            expect((await strategy.calculate(oneLanguage)).breakdown?.languageDiversity).toBe(40);
            expect((await strategy.calculate(noLanguage)).breakdown?.languageDiversity).toBe(20);
        });
        it('should score repository quality', async () => {
            ...
            const highResult = await strategy.calculate(highQuality);
            const lowResult = await strategy.calculate(lowQuality);
        ...
        it('should cap score at 100', async () => {
            ...
            const result = await strategy.calculate(hyperActiveCandidate);
...
//# sourceMappingURL=activity.strategy.spec.js.map