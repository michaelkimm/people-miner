...
describe('ScoringService', () => {
    ...
    beforeEach(async () => {
        ...
        const module = await testing_1.Test.createTestingModule({
            providers: [
                ...
                { provide: strategies_1.StrategyRegistry, useValue: mockStrategyRegistry },
            ],
        }).compile();
        service = module.get(scoring_service_1.ScoringService);
    });
    describe('scoreCandidate', () => {
        it('should score a candidate successfully', async () => {
            ...
            mockStrategyRegistry.getNormalizedWeights.mockReturnValue(new Map([['testStrategy', 1.0]]));
            mockStrategy.calculate.mockResolvedValueOnce({
                value: 80,
                breakdown: { metric1: 40, metric2: 40 },
            });
            mockStrategy.isApplicable.mockReturnValue(true);
        ...
        it('should throw when candidate not found', async () => {
            ...
            await expect(service.scoreCandidate('nonexistent')).rejects.toThrow('Candidate nonexistent not found');
        });
    });
    describe('calculateScore', () => {
        it('should calculate weighted scores from multiple strategies', async () => {
            ...
            mockStrategyRegistry.getNormalizedWeights.mockReturnValue(new Map([
                ['strategy1', 0.6],
                ['strategy2', 0.4],
            ]));
            mockPrisma.candidate.update.mockResolvedValueOnce({});
    ...
    describe('scoreAllCandidates', () => {
        ...
        it('should use force option to rescore all', async () => {
            ...
            expect(mockPrisma.candidate.findMany).toHaveBeenCalledWith(expect.objectContaining({
                where: {},
                take: 100,
            }));
        });
