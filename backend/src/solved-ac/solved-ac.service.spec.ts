...
import { SolvedAcService, SolvedAcUser } from './solved-ac.service';
...
describe('SolvedAcService', () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<SolvedAcService>(SolvedAcService);
  ...
  describe('getUserProfile', () => {
    ...
    it('should return user profile on success', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        'https://solved.ac/api/v3/user/show?handle=testuser',
      );
    });
  ...
  describe('fetchAndSaveProfile', () => {
    ...
    beforeEach(() => {
      // First call for getUserProfile, second for getUserTagStats
      mockFetch
  ...
  describe('syncCandidateSolvedAc', () => {
    ...
    it('should extract handle from bio and sync', async () => {
      ...
      // Mock getUserProfile and getUserTagStats
      mockFetch
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({
    ...
    it('should try github username when no handle in bio', async () => {
      ...
      // First call: check if GitHub username exists on solved.ac
      mockFetch
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({
        ...
        // Second call: fetchAndSaveProfile -> getUserProfile
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({
            handle: 'sameuser',
    ...
    it('should return false when no handle found anywhere', async () => {
      ...
      // GitHub username not found on solved.ac
      mockFetch.mockResolvedValueOnce({
        ok: false,
        status: 404,
      });
  ...
  describe('syncAllCandidates', () => {
    it('should sync multiple candidates', async () => {
      ...
      // Mock syncCandidateSolvedAc behavior
