...
async function main() {
  ...
  try {
    ...
    for (const candidate of candidates) {
      ...
      const hasMobileKeyword = MOBILE_KEYWORDS.some((kw) =>
      ...
      const hasOnlyMobileLanguages =
        uniqueLanguages.length > 0 &&
        uniqueLanguages.every(
          (lang) =>
            MOBILE_ONLY_LANGUAGES.map((l) => l.toLowerCase()).includes(
              lang.toLowerCase(),
            ) &&
            !BACKEND_LANGUAGES.map((l) => l.toLowerCase()).includes(
              lang.toLowerCase(),
            ),
        );
      ...
      if (hasOnlyMobileLanguages) {
        toDelete.push(candidate.id);
        console.log(
          `[DELETE] ${candidate.githubUsername}: only mobile languages (${uniqueLanguages.join(', ')})`,
        );
        continue;
      }

      const hasBackendLanguage = uniqueLanguages.some((lang) =>
        BACKEND_LANGUAGES.map((l) => l.toLowerCase()).includes(
          lang.toLowerCase(),
        ),
      );
      ...
      const hasAmbiguousLanguage = uniqueLanguages.some((lang) =>
        AMBIGUOUS_LANGUAGES.map((l) => l.toLowerCase()).includes(
          lang.toLowerCase(),
        ),
      );

      if (hasAmbiguousLanguage) {
        const hasBackendKeyword = BACKEND_KEYWORDS.some((kw) =>
          textContext.includes(kw.toLowerCase()),
        );
      ...
      toDelete.push(candidate.id);
      console.log(
        `[DELETE] ${candidate.githubUsername}: no backend indicators (languages: ${uniqueLanguages.join(', ')})`,
      );
    }
